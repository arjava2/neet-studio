...
<style>
    ...
    * { font-family: 'Inter', -apple-system, sans-serif !important; }
    ...
    h1 { font-size: 28px !important; font-weight: 800 !important; letter-spacing: -0.03em !important; color: #fafafa !important; margin-bottom: 2px !important; }
    h2 { font-size: 22px !important; font-weight: 700 !important; color: #fafafa !important; }
    h3 { font-size: 18px !important; font-weight: 700 !important; color: #fafafa !important; }
    
    footer, .stDeployButton, #MainMenu { display: none !important; visibility: hidden !important; }
    
    header[data-testid="stHeader"] { background: rgba(9,9,11,0.85) !important; backdrop-filter: blur(10px); border-bottom: 1px solid #27272a; }
    ...
    section[data-testid="stSidebar"] { background: #09090b !important; border-right: 1px solid #1c1c1f !important; }
    ...
    .stSelectbox > div > div {
        background: #18181b !important; border: 1px solid #27272a !important; border-radius: 8px !important;
    }
    ...
    .stButton > button[kind="primary"]:hover { box-shadow: 0 4px 16px rgba(99,102,241,0.4) !important; }
    ...
    [data-testid="stMetricLabel"] {
        color: #71717a !important; font-size: 11px !important; font-weight: 600 !important;
    ...
    .streamlit-expanderHeader {
        background: #18181b !important; border: 1px solid #27272a !important;
        border-radius: 8px !important; color: #fafafa !important; font-weight: 500 !important; font-size: 14px !important;
    }
    .streamlit-expanderHeader:hover { border-color: #3f3f46 !important; }
    ...
    .stTabs [data-baseweb="tab-list"] { background: transparent !important; border-bottom: 1px solid #27272a !important; }
    .stTabs [data-baseweb="tab"] {
        background: transparent !important; color: #71717a !important;
        font-weight: 500 !important; font-size: 14px !important; padding: 12px 16px !important;
    }
    ...
    .brand { font-size: 20px; font-weight: 800; background: linear-gradient(135deg, #818cf8, #a78bfa); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .sub { font-size: 12px; color: #52525b; margin-top: 2px; }
    .lbl { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: #52525b; margin: 20px 0 8px 0; }
    .crd { background: #18181b; border: 1px solid #27272a; border-radius: 10px; padding: 16px; margin-bottom: 12px; }
    .rw { display: flex; justify-content: space-between; padding: 5px 0; font-size: 13px; border-bottom: 1px solid #1c1c1f; }
    .rw:last-child { border: none; }
    .rl { color: #71717a; }
    .rv { color: #fafafa; font-weight: 500; }
    .pl { display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 600; }
    .pl-on { background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.3); color: #22c55e; }
    .pl-off { background: rgba(113,113,122,0.1); border: 1px solid rgba(113,113,122,0.3); color: #71717a; }
    .dt { width: 6px; height: 6px; border-radius: 50%; display: inline-block; }
    .dg { background: #22c55e; animation: bl 2s infinite; }
    .dr { background: #71717a; }
    @keyframes bl { 0%,100%{opacity:1} 50%{opacity:.5} }
    .op { padding: 10px 14px; margin: 4px 0; background: #18181b; border: 1px solid #27272a; border-radius: 8px; display: flex; align-items: center; gap: 10px; font-size: 14px; color: #fafafa; }
    .op-ok { background: rgba(34,197,94,0.06); border-color: rgba(34,197,94,0.3); }
    ...
    .mk-no { background: #27272a; color: #71717a; }
    ...
    .tg-e { background: rgba(34,197,94,0.12); color: #22c55e; }
    .tg-m { background: rgba(234,179,8,0.12); color: #eab308; }
    .tg-h { background: rgba(239,68,68,0.12); color: #ef4444; }
    .tg-t { background: rgba(99,102,241,0.12); color: #818cf8; }
    .emp { text-align: center; padding: 50px 20px; background: #18181b; border: 1px dashed #27272a; border-radius: 10px; color: #fafafa; }
    .emp p { color: #71717a !important; }
</style>
...
with st.sidebar:
    ...
    st.markdown('<div class="sub">Question generation platform</div>', unsafe_allow_html=True)
...
if not st.session_state.ai_ready:
    ...
    with c1:
        st.markdown('<div class="crd"><div style="font-size:24px;margin-bottom:8px;">⚡</div><div style="font-weight:600;">Fast</div><p style="color:#71717a;font-size:12px;margin:4px 0 0 0;">30 questions in 60 seconds</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="crd"><div style="font-size:24px;margin-bottom:8px;">🎯</div><div style="font-weight:600;">Verified</div><p style="color:#71717a;font-size:12px;margin:4px 0 0 0;">AI double-checks answers</p></div>', unsafe_allow_html=True)
    with c3:
        st.markdown('<div class="crd"><div style="font-size:24px;margin-bottom:8px;">🌐</div><div style="font-weight:600;">Bilingual</div><p style="color:#71717a;font-size:12px;margin:4px 0 0 0;">English + Hindi output</p></div>', unsafe_allow_html=True)
    st.markdown("### How to use")
    st.markdown("1. Get free API key from [aistudio.google.com/apikey](https://aistudio.google.com/apikey)")
    st.markdown("2. Paste key in sidebar and click Connect")
    st.markdown("3. Upload PDFs or use pre-loaded library")
    st.markdown("4. Select chapter and generate questions!")
    st.stop()
...
with tab1:
    ...
    if st.session_state.generated:
        ...
        with h1:
            st.markdown("### Results")
            st.caption(f"{len(qs)} questions generated, {vc} verified")
        with h2:
        ...
        for i, q in enumerate(qs):
            ...
            with st.expander(f"{icon}Q{i+1} - {q.get('question_english','')[:65]}..."):
                ...
                opt_pairs_en = [('A', 'option_a_english'), ('B', 'option_b_english'), ('C', 'option_c_english'), ('D', 'option_d_english')]
                opt_pairs_hi = [('A', 'option_a_hindi'), ('B', 'option_b_hindi'), ('C', 'option_c_hindi'), ('D', 'option_d_hindi')]
                with t1:
                    st.markdown(f"**{q.get('question_english','')}**")
                    for o, k in opt_pairs_en:
                        ok = o == correct
                        rc = "op op-ok" if ok else "op"
                        mc = "mk mk-ok" if ok else "mk mk-no"
                        st.markdown(f'<div class="{rc}"><span class="{mc}">{o}</span><span>{q.get(k,"")}</span></div>', unsafe_allow_html=True)
                    st.info(q.get('explanation_english', ''))
                with t2:
                    st.markdown(f"**{q.get('question_hindi','')}**")
                    for o, k in opt_pairs_hi:
                        ok = o == correct
                        rc = "op op-ok" if ok else "op"
                        mc = "mk mk-ok" if ok else "mk mk-no"
                        st.markdown(f'<div class="{rc}"><span class="{mc}">{o}</span><span>{q.get(k,"")}</span></div>', unsafe_allow_html=True)
                ...
                with b2:
                    if st.button("Humanize", key=f"h_{i}", use_container_width=True):
                        ...
                        st.rerun()
                with b3:
                    if st.button("Meta", key=f"m_{i}", use_container_width=True):
                ...
                if vd:
                    ...
                    if vd.get("matches") and cf >= 80:
                        ...
                        st.warning(f"Likely correct - {cf}%")
...
with tab4:
    ...
    if total == 0:
        ...
        if not ext:
            st.info("Click Extract All above")
        else:
